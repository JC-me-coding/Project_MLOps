...
def training(config):
    ...
    train_loader = load_data(
        f"{root}/data/processed/landscapes", "train", batch_size, config.data
    )
    valid_loader = load_data(
        f"{root}/data/processed/landscapes", "val", batch_size, config.data
    )
    ...
    wandb.init(
        project=config.wandb.project, entity=config.wandb.entity, config=wandb_config
    )
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_accuracy = train_step(
            net, loss_function, optimizer, train_loader, device, epoch
        )

        if epoch % val_interval == 0:
            val_loss, val_accuracy = val_step(
                net, loss_function, valid_loader, device, epoch
            )
            if val_accuracy > best_val_acc:
                best_val_acc = val_accuracy
                torch.save(
                    net.state_dict(),
                    f"./models/{backbone}_{config.hyperparameters.learning_rate}.pth",
                )
...
if __name__ == "__main__":
    ...
    if config.hyperparameters.sweep_config == "":
        ...
        sweep_config = yaml.load(
            open(config.hyperparameters.sweep_config, "r"), Loader=yaml.FullLoader
        )
        sweep_id = wandb.sweep(
            sweep=sweep_config,
            project="MLOPsproject-Sweep",
